                    ...
                    if sub_end > clip_start and sub_start < clip_end:
                        ...
                        if relative_end > relative_start:
                            ...
                            for i, word in enumerate(words):
                                ...
                                # Create cool animated gradient text effect
                                def create_animated_text_clip(word, font_size, duration):
                                    """Create a text clip with enhanced styling"""
                                    # Enhanced styling options with better fonts and gradient effects
                                    # Use fonts that actually work with MoviePy
                                    font_options = [
                                        'DUBAI-BOLD.TTF',   # Bold, modern font
                                        'calibri.ttf',      # Modern, clean
                                        'verdana.ttf',      # Clean, readable  
                                        'arial.ttf',        # Classic fallback
                                        'arial.ttf'         # Final fallback
                                    ]
                                    
                                    # Try to find an available font - only test once per word
                                    if i == 0:  # Only test fonts for the first word
                                        selected_font = 'arial.ttf'  # Default fallback
                                        for font in font_options:
                                            try:
                                                # Test if font is available by trying to create a small text clip
                                                test_clip = TextClip(
                                                    text="Test",
                                                    font_size=10,
                                                    font=font,
                                        ...
                                        if selected_font == 'arial.ttf':
                                            print(f"  Using default font: arial.ttf")
                                    
                                    # Use the selected font for all words
                                    selected_font = getattr(create_animated_text_clip, 'selected_font', 'arial.ttf')
                                    ...
                                    styling_options = [
                                        ...
                                        },
                                        {
                                            ...
                                            'stroke_width': 8,
                                            'font': selected_font,
                                            'font_size_modifier': 1.3,
                                            'weight': 'bold'
                                        },
                                        {
                                            'color': '#00f',  # Blue again for gradient effect
                                            'stroke_color': '#000',
                                            'stroke_width': 8, 
                                            'font': selected_font,
                                    ...
                                    # Choose styling based on word position for gradient effect
                                    style_idx = i % len(styling_options)
                                    style = styling_options[style_idx]
                                    
                                    # Apply font size modifier
                                    adjusted_font_size = int(font_size * style['font_size_modifier'])
                                    
                                    # Create enhanced text clip with bold styling
                                    txt_clip = TextClip(
                                        text=word,
                                    ...
                                    try:
                                        ...
                                        # Composite glow with main text for better effect
                                        final_clip = CompositeVideoClip([glow_clip, txt_clip])
                                        return final_clip
                                    except:
                                        # Fallback to main clip if glow fails
                                        return txt_clip
                                
                                txt_clip = create_animated_text_clip(word, font_size, word_duration)
