...
def test_video_editing():
    ...
    # Test files
    video_path = "media_src/test_input_video.mp4"
    subtitle_path = "media_src/demo_subtitles.srt"
    
    # Create unique output filename with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = "test_outputs"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"test_output_{timestamp}.mp4")
    ...
    try:
        ...
        if os.path.exists(output_path):
            ...
            test_outputs = [f for f in os.listdir(output_dir) if f.startswith('test_output_') and f.endswith('.mp4')]
            ...
            for i, output in enumerate(test_outputs[:5]):  # Show last 5
                size = os.path.getsize(os.path.join(output_dir, output)) / (1024 * 1024)
